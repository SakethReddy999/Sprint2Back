...
public class ControllerClass {
	...
	public ResponseEntity<String> Ticketgeneration(@RequestBody Ticket mv) {
		Ticket ticket = serviceobj.Ticketgeneration(mv);
		if (ticket == null) {
	...
	private ResponseEntity<Ticket> getTicket(@PathVariable("id") int id) {
		Ticket ticket = serviceobj.getTicketById(id);
		if (ticket == null) {
			throw new IdNotFoundException("Id does not exist,so we couldn't fetch details");
		} else {
			return new ResponseEntity<Ticket>(ticket, new HttpHeaders(), HttpStatus.OK);
		}
	}
